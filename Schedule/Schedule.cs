...
namespace Schedule
{
    public class Schedule
    {
        ...
        public static string SQLSchedule0001 = $"" +
            ...
            $"INNER JOIN T_PRIOR USING(PRIORID) " +
            $"LEFT OUTER JOIN(SELECT SCHEDULEID,'！' AS ALERTSTATUS FROM T_SCHEDULE WHERE STATUSID in (1,2) AND SCHEDULEENDTIME<{now}) USING(SCHEDULEID) " +
            $"WHERE SCHEDULEVISIBLESTATUS=1 ORDER BY SCHEDULEDATE DESC,SCHEDULESTARTTIME,SCHEDULEENDTIME LIMIT {SQLLimit}";
        public static string SQLSchedule0002 = $"" +
            ...
            $"ORDER BY SCHEDULEDATE DESC,SCHEDULESTARTTIME,SCHEDULEENDTIME LIMIT {SQLLimit}";
        ...
        public static string SQLSchedule0004 = $"" +
            ...
            $"ORDER BY SCHEDULEDATE DESC,SCHEDULESTARTTIME,SCHEDULEENDTIME";
        public static string SQLSchedule0005 = "" +
            "SELECT SCHEDULESTARTTIME,SCHEDULEENDTIME FROM T_SCHEDULE WHERE SCHEDULEID != @SCHEDULEID AND SCHEDULEDATE=@SCHEDULEDATE";
