...
namespace Note
{
    public class Note
    {
        ...
        public static string SQLNote0000 = "" +
            "SELECT NOTETITLEID,SOURCEID,KINDID,'★最新' AS NOTEVERSION,NOTETITLE,NOTETEXT,strftime('%Y-%m-%d',NOTEDATE) AS NOTEDATE,ROWID,FIN FROM T_NOTETITLE " +
        ...
        public static string SQLNote0001 = "" +
            "SELECT NOTETITLEID,SOURCEID,KINDID,CASE WHEN T_NOTETITLE.NOTEVERSION='0' THEN '★最新' ELSE T_NOTETITLE.NOTEVERSION END AS NOTEVERSION,NOTETITLE,NOTETEXT,strftime('%Y-%m-%d',NOTEDATE) AS NOTEDATE,ROWID,FIN FROM T_NOTETITLE " +
        ...
        public static string SQLNote0002 = $"" +
            $"SELECT NOTETITLEID,SOURCEID,KINDID,'★最新' AS NOTEVERSION,CASE WHEN NOTETYPE='0' THEN '【ノート】' WHEN NOTETYPE='1' THEN '【履歴】' END AS NOTETYPE,NOTETITLE,NOTETEXT,strftime('%Y-%m-%d',NOTEDATE) AS NOTEDATE,ROWID,FIN FROM T_NOTETITLE " +
        ...
        public static string SQLNote0007 = "" +
            "SELECT strftime('%Y-%m-%d',NOTEDATE) AS NOTEDATE,NOTETITLE,NOTETEXT,ROWID,FIN FROM T_NOTETITLE INNER JOIN T_NOTETEXT USING (NOTETITLEID) WHERE T_NOTETITLE.NOTEVERSION = 0 AND (SOURCEID=@NOTETITLEID OR NOTETITLEID=@NOTETITLEID) ORDER BY ROWID";
