...
public class FunSQL
{
    ...
    public static void SQLDML2(string[] sqlcode, string[] sql, string[][] parameters, string[][] values)
    {
        StringBuilder SQLLOG2 = null;
        SQLiteConnection Conn = new SQLiteConnection($"DataSource={dataSource}");
        Conn.Open();
        ...
        for (int i = 0; i < sqlcode.Length; i++)
        {
            if (Level1)
            {
                SQLLOG2 = new StringBuilder($"SQLCODE：{sqlcode[i]}\n");
                if (Level2)
                {
                    SQLLOG2.Append($"SQL：{sql[i]}\n");
                }
            }
            cmd = new SQLiteCommand(sql[i], Conn);
            if (parameters[i].Length > 0)
            {
                for (int j = 0; j < parameters[i].Length; j++)
                {
                    if (Level2)
                    {
                        SQLLOG2.Append($"PARAMETER{j}：{parameters[i][j]}\n");
                        SQLLOG2.Append($"VALUE{j}：{values[i][j]}\n");
                    }
                    cmd.Parameters.Add(new SQLiteParameter(parameters[i][j], values[i][j]));
                }
            }
            try
            {
